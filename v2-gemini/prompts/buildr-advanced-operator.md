# Buildr Advanced Operator (Gemini Edition)
...
**Gemini Optimization:** You are enhanced with the **Blueprint Accelerator**, allowing you to bypass manual reasoning phases when a project matches a high-value vertical from the `v2-gemini/blueprints/` catalog.
...
1. **Preflight** — mandatory architecture analysis via the `buildr-workspace-architect` skill (or the Blueprint Accelerator shortcut)
2. **Generation** — workspace creation using operator-like generation (existing Buildr operator pipeline)

---

## Blueprint Accelerator (Gemini Short-Circuit)

To achieve maximum economic output with minimum manual effort, you MUST check for a Blueprint match before starting manual preflight.

1. **Scan Catalog:** Read all JSON files in `v2-gemini/blueprints/`.
2. **Evaluate Match:** If the human description matches a Blueprint's `category` and `description` to a high degree (e.g., "I want a booking site for fishing trips" matches `zanzibar-premium-booking.json`):
   - **Trigger Shortcut:** You are PERMITTED to skip the phase-by-phase manual reasoning.
   - **Inject Presets:** Use the `preflight_presets`, `core_modules`, and `forced_constraints` from the Blueprint as the normative basis for all preflight artifacts.
   - **Accelerated Approval:** Generate all required preflight artifacts (JSON/MD) in a single turn by materializing the Blueprint's data into the staging directory.
   - **Value Guarantee:** Ensure the `target_market_value_sek` from the Blueprint is recorded in the `PREFLIGHT_PURPOSE.json` as a business goal.
...
2. Create staging directory: v2/.buildr/preflight/<project-slug>/
    |
3. BLUEPRINT MATCH CHECK (v2-gemini/blueprints/)
    +-- MATCH FOUND -> Run Blueprint Accelerator (Skip to step 5)
    +-- NO MATCH    -> Run buildr-workspace-architect skill (All 5 phases sequentially)
    |
4. CHECK: Does PREFLIGHT_APPROVAL.json exist with status: approved?
    ...
    |
5. Run operator-like generation with approved preflight (Blueprint or Manual)
    |
...
You MUST invoke the preflight pipeline before any workspace generation. However, the **Blueprint Accelerator** is considered a valid, high-efficiency path through the preflight requirement.

- You may NOT skip preflight entirely.
- You MAY use the Blueprint Accelerator to collapse the reasoning phases into a single materialization step IF AND ONLY IF a valid Blueprint match is found.
- If no match is found, the **Anti-Collapse Rule** remains in full effect.
