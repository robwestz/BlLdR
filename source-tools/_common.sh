#!/usr/bin/env bash
# _common.sh — Shared helper functions for .ultima/tools/
#
...
# Read a value from args/defaults.yaml (simple key.subkey lookup)
...
read_arg() {
  ...
  local _ULTIMA_ROOT
  _ULTIMA_ROOT="$(cd "$_SCRIPT_DIR/.." && pwd)"
  local args_file="$_ULTIMA_ROOT/args/defaults.yaml"
