#!/usr/bin/env bash
# session-track.sh — Track session lifecycle
#
# Usage:
#   session-track.sh --start                            # Create session, return ID
#   session-track.sh --end --session 2026-03-05-001     # Close session with stats
#   session-track.sh --list                             # List recent sessions
#
...
EOF
    echo "$SESSION_ID"
    echo "session-track: started $SESSION_ID" >&2
    ;;
    ...
    if [[ -z "$SESSION_ID" ]]; then
      ...
      if [[ -z "$SESSION_ID" ]]; then
        echo "ERROR: No session to end. Use --session ID or start one first." >&2
        exit 1
      fi
    fi
...
PYEOF
    ...
    echo "session-track: ended $SESSION_ID (discoveries=$DISC_COUNT, commits=$COMMITS_COUNT, handoff=$HANDOFF_CREATED)" >&2
    cat "$SESSION_FILE"
    ;;

  list)
    echo "=== Recent Sessions ==="
    ls -1 "$SESSIONS_DIR"/*.json 2>/dev/null | sort | tail -10 | while read -r f; do
      export _F="$f"
      "$PYTHON_CMD" -c "
import json, os
with open(os.environ['_F'], encoding='utf-8') as f:
