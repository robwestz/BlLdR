#!/usr/bin/env bash
# discovery-mine.sh — Mine git commits for discoveries (0 tokens)
#
...
#
# Usage:
#   discovery-mine.sh                # Mine new commits since last discovery
#   discovery-mine.sh --since DATE   # Override start date (ISO 8601)
#   discovery-mine.sh --dry-run      # Show what would be written
#
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      echo "Usage: discovery-mine.sh [--since DATE] [--dry-run]" >&2
      exit 1
      ;;
  esac
done
...
export _BUILDR_DISC="$DISCOVERIES"
export _BUILDR_DRY_RUN="$DRY_RUN"
export _BUILDR_GIT_ROOT="$GIT_ROOT"
export _BUILDR_EXISTING="$EXISTING_HASHES"
export _BUILDR_COMMITS="$COMMITS"

"$PYTHON_CMD" << 'PYEOF'
import sys, json, os, subprocess
from datetime import datetime, timezone

discoveries_path = os.environ['_BUILDR_DISC']
dry_run = os.environ['_BUILDR_DRY_RUN'] == 'true'
git_root = os.environ['_BUILDR_GIT_ROOT']
existing_set = set(os.environ['_BUILDR_EXISTING'].split())
commits_raw = os.environ.get('_BUILDR_COMMITS', '')
...
print(f"discovery-mine: {action} {new_count} discoveries from git commits", file=sys.stderr)
PYEOF

unset _BUILDR_DISC _BUILDR_DRY_RUN _BUILDR_GIT_ROOT _BUILDR_EXISTING _BUILDR_COMMITS
