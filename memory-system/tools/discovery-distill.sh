#!/usr/bin/env bash
# discovery-distill.sh — Compress raw discoveries into topic summaries
#
# Reads all discoveries, groups by topic, and produces a compact summary
# per topic. This is the "warm" tier — more context than wave-brief,
# but far less than reading all raw discoveries.
#
# Pattern: Raw JSONL (cold) → Distilled summary (warm) → Wave brief (hot)
#
# Usage:
#   discovery-distill.sh                              # Output to stdout
#   discovery-distill.sh > memory-system/context/distilled.md
#   discovery-distill.sh --max-per-topic 5            # Limit entries per topic
#   discovery-distill.sh --max-total 20               # Cap total output lines
#
...
if [[ ! -f "$DISCOVERIES" ]]; then
  ...
  echo ""
  echo "_No discoveries to distill._"
  exit 0
fi
...
DEFAULTS_JSON="$MEMORY_ROOT/config/defaults.json"
DEFAULTS_W=$(winpath "$DEFAULTS_JSON")

export _BUILDR_DISC="$DISCOVERIES_W"
export _BUILDR_DEFAULTS="$DEFAULTS_W"
export _BUILDR_MAX="$MAX_PER_TOPIC"
export _BUILDR_MAX_TOTAL="$MAX_TOTAL"
...
max_per_topic = int(os.environ['_BUILDR_MAX'])
max_total = int(os.environ['_BUILDR_MAX_TOTAL'])
...
IDENTITY_WEIGHTS = {}
defaults_path = os.environ.get('_BUILDR_DEFAULTS', '')
if defaults_path:
    try:
...
for topic in topic_order:
    if lines_emitted >= max_total:
        ...
        print(f"_({remaining} older topics omitted — max {max_total} lines)_")
        break
...
print(f"_Distilled: {now_str} UTC | {lines_emitted} lines | max {max_total}_")
PYEOF

unset _BUILDR_DISC _BUILDR_MAX _BUILDR_MAX_TOTAL
